...
class Wav2Vec2Encoder(nn.Module):
    ...
    def forward(
        ...
        if noise_layer is not None and noise_layer == 0:
#             print("Injecting noise", noise, "into layer 0")
            hidden_states += hidden_states * (torch.randn_like(hidden_states) * noise)
        ...
        for layer in self.layers:
            ...
            # Injecting noise
            if noise_layer is not None and noise_layer == counter:
#                 print("Injecting noise", noise, "into layer", counter)
                hidden_states += hidden_states * (torch.randn_like(hidden_states) * noise)
            counter += 1
